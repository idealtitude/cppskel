...
def get_args(cppskel_path: str) -> Any:
    ...
    parser.add_argument("-b", "--benchmark", action="store_true", help="Optional benchmarking utility")
    parser.add_argument(
...
def main(arguments: list[str]) -> int:
    ...
    benchmark_string  : str = ''

    if args.benchmark:
        # Benchmarking it...
        benchmark_perf = perf_counter()
        benchmark_process = process_time()
    ...
    if args.name:
        ...
        if setup_skeleton(clean_name):
            if args.benchmark:
                final_perf_time = perf_counter() - benchmark_perf
                final_proc_time = process_time() - benchmark_process
                benchmark_string = f"\n\033[1mExecution time:\033[0m\t{round(final_perf_time, 5)} seconds\n\033[1mProcess time:\033[0m\t{round(final_proc_time, 5)} seconds"
            print(f"\033[92mSuccess:\033[0m Skeleton setup done with name \"{clean_name}\", in location:\n{os.path.join(APP_CWD, clean_name)}{benchmark_string}")
